...
export class BestImagePlugin extends PluginInterface {
	...
	private static _collectCandidates(
		...
		const register = (entry: any, source: string): void => {
			for (const normalized of BestImagePlugin._normalizeEntries(entry)) {
				BestImagePlugin._mergeCandidate(candidates, normalized, source);
			}
		};
		...
		BestImagePlugin._collectOpenGraphCandidates(scraper, candidates);
	...
	private static *_normalizeEntries(entry: any): Generator<ImageCandidate> {
		...
		if (Array.isArray(entry)) {
			for (const item of entry) {
				yield* BestImagePlugin._normalizeEntries(item);
			}
			return;
		}
	...
	private static _collectOpenGraphCandidates(
		...
		for (const imageData of ogImageData) {
			if (imageData.url) {
				BestImagePlugin._mergeCandidate(
					candidates,
					{
						url: imageData.url,
	...
	private static _mergeCandidate(
		...
		const width = BestImagePlugin._parseDimension(candidate.width);
		const height = BestImagePlugin._parseDimension(candidate.height);
		...
		if (width !== null) {
			existing.width = BestImagePlugin._maxDimension(existing.width, width);
		}
		if (height !== null) {
			existing.height = BestImagePlugin._maxDimension(existing.height, height);
		}
	...
	private static _selectBestCandidate(
		...
		for (const candidate of candidates) {
			const score = BestImagePlugin._scoreCandidate(candidate);
			if (
				score[0] > bestScore[0] ||
	...
	private static _ensureDimensions(
		...
		const dims = BestImagePlugin._extractDimensionsFromUrl(candidate.url);
		if (dims) {
			width = width ?? dims[0];
	...
	private static _extractDimensionsFromUrl(
		...
		const widthMatch = url.match(BestImagePlugin.QUERY_WIDTH_PATTERN);
