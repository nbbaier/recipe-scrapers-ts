...
function extractHostname(filepath: string): string | null {
  ...
  const hostMatch = content.match(/host\(\):\s*string\s*\{\s*return\s+"([^"]+)"/);
  if (hostMatch) {
    return hostMatch[1];
  }
  return null;
...
function main() {
  ...
  const importStart = factoryContent.indexOf('// Import site-specific scrapers');
  const importEnd = factoryContent.indexOf('// Register all site-specific scrapers');
  ...
  const registrations = scrapers.map(s => {
