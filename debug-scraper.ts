...
const html = readFileSync('../tests/test_data/allrecipes.com/allrecipescurated.testhtml', 'utf-8');

const scraper = scrapeHtml(html, 'https://allrecipes.com/', { supportedOnly: true });
...
console.log('Schema data keys:', Object.keys((scraper as any).schema?.data || {}));
console.log('Schema has author method:', typeof (scraper as any).schema?.author);
...
try {
  console.log('opengraph.title():', typeof (scraper as any).opengraph.title === 'function' ? (scraper as any).opengraph.title() : (scraper as any).opengraph.title);
} catch (e: any) {
  console.log('opengraph.title() error:', e.message);
}
console.log('opengraph.image():', typeof (scraper as any).opengraph.image === 'function' ? (scraper as any).opengraph.image() : 'not a function');
console.log('opengraph.article_author:', (scraper as any).opengraph.article_author);
